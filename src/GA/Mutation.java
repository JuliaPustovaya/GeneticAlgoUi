...
public class Mutation {
	...
	/*
		public static ArrayList<Xromocoma> RandomMutation(ArrayList<Xromocoma> list) {
			...
			System.out.println("Модифицированные потомки ");
			for (int i = 0; i <list.size() ; i++) {
		...
		public static double getModifiedGene(double value, int numberOfCurrentXromocoma) {
			...
			int valueMin = 0;
			if (r > 0.5) {
				result = value + (valueMax - value) * r * (1.0 - Math.pow((numberOfCurrentXromocoma / (double) POPULATION), 2));
			} else {
				result = value + (value - valueMin) * r * (1.0 - Math.pow((numberOfCurrentXromocoma / (double) POPULATION), 2));
			}
			return result;
		}
	*/
